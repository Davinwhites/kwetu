...
- **This app's AI calls (`src/lib/ai.ts`) run on Google Gemini, not xAI** —
  swapped deliberately so the app owner pays nothing to run AI: Gemini 2.5
  Flash's free tier is rate-limited, not metered/billed. Key is
  `GEMINI_API_KEY` (get one with no card at aistudio.google.com), read the
  same way as `XAI_API_KEY` above. Access to the AI features inside the app
  is still gated by `src/lib/billing/plans.ts` (subscribers only) — that's
  the app's own paywall and is unrelated to which provider is being billed.
